          ...
          <Reveal>
            <div className="card grid gap-8 bg-ink p-6 text-white sm:p-10 md:grid-cols-[1fr_1fr] md:items-center md:p-14">
              ...
              <div className="grid gap-3">
                {[[Fence, "Found something?", "Signboard install, fencing, boundary marking or a lawyer's notice — verified pros, quote + 10%, you approve."], [ShieldCheck, "Same trust chain as homes", "Police-verified inspector, GPS + time on every photo, protected up to ₹1,00,000."], [Clock, "Report within the hour", "Boundary walk, photo map and every corner — in your inbox the same morning."]].map(([I, t, b]) => { const Icon = I as typeof Fence; return <div key={t as string} className="flex gap-3 rounded-[14px] bg-white/[0.07] p-4"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/10"><Icon size={16} /></span><div><div className="text-[15px] font-medium">{t as string}</div><div className="mt-1 text-[13px] text-white/65">{b as string}</div></div></div>; })}
              </div>
            </div>
          </Reveal>
        </div>
