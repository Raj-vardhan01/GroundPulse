...
import { devSignIn, type FormState } from "@/lib/actions";
import { SubmitButton } from "@/components/app/SubmitButton";

const ERRORS: Record<string, string> = {
  "google-off": "Google sign-in is not switched on yet. Please try again soon, or write to us.",
...
/* Owners sign in with Google — no password, no code to wait for. A phone
   number is asked for straight after, on the welcome screen. */
export function OwnerDoor({ googleOn, dev, error }: { googleOn: boolean; dev: boolean; error?: string }) {
  return (
    <div className="w-full">
      <h1 className="serif text-[clamp(2rem,4vw,2.6rem)] leading-[1.05] tracking-[-0.035em]">Sign in.</h1>
      <p className="t-small mt-2.5">Use your Google account — no password to forget at 2 AM in another timezone. We ask for your phone number next, so the inspector can call you from the door.</p>
      ...
      {googleOn ? (
        ...
        <p className="mt-7 rounded-[12px] border border-line-2 bg-white px-4 py-3.5 text-[13.5px] text-text-2">Google sign-in is being switched on. {dev ? "Use the development sign-in below." : "Please try again soon, or write to us."}</p>
      )}
  ...
  return (
    <form action={act} className="mt-6 rounded-[14px] border border-gold/40 bg-gold-soft p-4">
      <p className="flex items-start gap-2 text-[12.5px] leading-snug text-[#7a5209]">
        <FlaskConical size={14} className="mt-0.5 shrink-0" /> Development only — sign in as any owner by email. Production never shows this.
      </p>
